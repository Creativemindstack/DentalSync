...
function getSavedClinicId() {
  return localStorage.getItem('messageClinicId');
}

function setSavedClinicId(value) {
  if (!value) return;
  localStorage.setItem('messageClinicId', value);
}

function populateClinicSelect() {
  if (!clinicSelect || typeof DentalSync === 'undefined') return;
  const previous = clinicSelect.value || getSavedClinicId();
  ...
  if (previous && [...clinicSelect.options].some(opt => opt.value === previous)) {
    clinicSelect.value = previous;
  } else if (clinicSelect.options.length) {
...
if (patientComposeForm) {
  ...
  patientComposeForm.addEventListener('submit', e => {
    ...
    if (!clinic) {
      showToast('Selected clinic not found.', 'warn');
      return;
    }
...
function renderMessageThreads() {
  ...
  threads.forEach(thread => {
    ...
    const clinicInfo = root.clinicName ? `${root.dentist ? root.dentist + ' · ' : ''}${root.clinicName}` : (root.dentist || 'Clinic');
    card.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:12px;">
        <div style="display:flex;gap:12px;flex:1;">
          ...
          <div style="flex:1;">
            ...
            <p class="notif-time">${clinicInfo}${thread.unread ? ' · Unread' : ''}</p>
          </div>
        </div>
